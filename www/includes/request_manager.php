...
*******************************************************************************/
	private function serverLocked($log = false) {
		...
		if ($locked) { logMessage('Server is locked ! Cannot save for now...'); }
		if (!$locked) { logMessage('Server is unlocked ! Can save !'); }
		return $locked;
	}
	...
	private function getTokensForOrders($orders) {
		...
		foreach($orders as $eachOrder) {
			...
			if ($order->havePath()) {
		        ...
		        if ($highestCol === null || $orderHighestCol > $highestCol) { $highestCol = $orderHighestCol; }
		        if ($lowestCol === null || $orderLowestCol < $lowestCol) { $lowestCol = $orderLowestCol; }
		        if ($highestRow === null || $orderHighestRow > $highestRow) { $highestRow = $orderHighestRow; }
		        if ($lowestRow === null || $orderLowestRow < $lowestRow) { $lowestRow = $orderLowestRow;  }
...
*******************************************************************************/
	...
	private function handleGetOrder($postData) {
		...
		if ($player===null) { logMessage("Nope, player unknown...");return []; }
		if ($object===null) { logMessage("Nope, object unknown...");return []; }
...
*/
    ...
    public function handleScopeRequest($postData, $what) {
	    ...
	    if (preg_match('/^(-?\d+),(-?\d+),(-?\d+),(-?\d+)$/', $postData['M'], $matches)) {
	        ...
	        if ($col2 < $col1) { [$col1, $col2] = [$col2, $col1]; }
	    } else {
	    ...
	    foreach ($to_load_coords as $key => [$col, $row]) {
	        if ($what === 'Ground' || $what === 'All') $cache_results['Ground'][$key] = $this->board->getGround($col, $row);
	        if ($what === 'City' || $what === 'All')  $cache_results['City'][$key] = $this->board->getObjectsByCoords('City', $col, $row);
	        if ($what === 'Unit' || $what === 'All')  $cache_results['Unit'][$key] = $this->board->getObjectsByCoords('Unit', $col, $row);
...
*******************************************************************************/
    ...
    private function objectsToPacket($ground, $cities = [], $units = []) {
		...
		$packet = [];
		$packet['c']=$ground->col; $packet['r']=$ground->row;
		$packet['o']=[];
