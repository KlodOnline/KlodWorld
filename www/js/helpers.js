...
var Chronographe = function() {
    ...
    var myTimer = function(name) {
        ...
        this.stop_time = null;
        this.steps = [];
        this.stop = function() { this.stop_time = performance.now(); }        
        this.step = function() { this.steps.push(performance.now()); }
        this.log = function(min_time) {
            if ((this.stop_time - this.start_time)<min_time) {return false;}
...
//            logMessage(' Launched @'+this.start_time);
    ...
    this.start = function(name) { timer = new myTimer(name); this.chrono_timers[name] = timer; }
    this.stop = function(name) { this.chrono_timers[name].stop(); }
    this.step = function(name) { this.chrono_timers[name].step(); }
