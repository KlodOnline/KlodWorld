...
class CityCartouche extends Cartouche {
    ...
    drawInside() {
	    ...
	    if (this.city.units == null) { return; }
...
class UnitCartouche extends Cartouche {
    ...
    drawMain() {
        ...
        for (let unit of this.units) {
            $('#u-' + this.tag).prepend(`<img id="${unit.id}" class="tok US ${helper_color_class(unit.co)}" src="./u/u${unit.ty}.png" />`);
        }
    }
    drawInside() {
...
class Selection {
	...
	digestData(data) {
	    ...
	    if (city) { this.selectedEntities = [city]; }
	    // Otherwise, use the units
	    else if (units.length > 0) { this.selectedEntities = units; }
	...
	genUnitCartouches(hash) {
	    ...
	    for (const unit of Object.values(this.selectedEntities)) {
	        if (unit.or == null) {
		        ...
		        if (!withOrders[orderHash]) { withOrders[orderHash] = []; }
		        withOrders[orderHash].push(unit);
	        }
	    }

	    logMessage('Found : noO='+JSON.stringify(noOrders) +' / prepO='+JSON.stringify(prepOrders) +' / wO='+JSON.stringify(withOrders));
...
==============================================================================*/
	...
	select(entities) {
		...
		for (let entity of entities) {
			...
			$('#selection').html('<div id="selection_container"><div id="selection_display">Loading...</div></div>');
	...
	cancelTarget() {
		// allowLogs();
		if (!this.currentPreparation()) {return false;}
		if (!this.currentOrder.use_path)  {return false;}
		this.target = null;
		Mv.set_path(this.currentOrder.path);
		// disableLogs();
	}
	...
	setTarget(target) {
		...
		if (this.last_set == currentTarget) { return false; }
		if (!this.currentPreparation()) {return false;}
		if (!this.currentOrder.use_path)  {return false;}
	...
	cancelOrder(cart_tag) {
		...
		for (const unit of units) {
		    if (unit.hasOwnProperty("or")) { delete unit.or; }
		}
    ...
    serverRefresh() {
    	// allowLogs();
		// A thing to code.
    	logMessage('Seems Like something happend, a turn change or something, but I do not know exactly what its expecting of me');
    	// disableLogs();
    }
...
$('#selection').contextmenu(function() { return false; });
