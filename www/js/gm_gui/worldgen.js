...
----------------------------------------------------------------------------- */
function FullCreation() {
  ajaxExecutor('FullCreation')
}
function createTectonicPlates() {
  ajaxExecutor('createTectonicPlates')
}
function loadWorld() {
  ajaxExecutor('loadWorld')
}
function clearWorld() {
  ajaxExecutor('clearWorld')
}
function Humidity() {
  ajaxExecutor('Humidity')
}
function LatitudeClimate() {
  ajaxExecutor('LatitudeClimate')
}
function Rivers() {
  ajaxExecutor('Rivers')
}
function DesertsCoasts() {
  ajaxExecutor('DesertsCoasts')
}
function LessBoring() {
  ajaxExecutor('LessBoring')
}
  ...
  function updateRowProgress() {
    if (currentRow <= maxRow) {
      for (let currentCol = 0; currentCol <= maxCol; currentCol++) {
        ...
        ctx.fillRect(currentCol * cellSize + offsetX, currentRow * cellSize, cellSize, cellSize)
      }
      currentRow++
      requestAnimationFrame(updateRowProgress)
    } else {
