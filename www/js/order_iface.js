...
function MOVE() {
  ...
  this.prep_txt = function () {
    ...
    const full_txt =
      '<br/> End of Travel : ' + time_to_txt(tic_to_date(this.tic_fulltmppath() + Mb.last_turn))
...
const filter_path = function (path) {
  ...
  for (var key in path) {
    ...
    if (step_coord.stg() !== last_ok.stg()) {
      clean_path.push(step_coord)
    }
  }
  return clean_path
}
...
const tic_fulltmppath = function () {
  ...
  for (const key in this.path) {
    if (typeof this.path[key - 1] !== 'undefined') {
      from_coord = this.path[key - 1]
    } else {
      continue
    }
    let move_cost = Mv.move_cost(from_coord, this.path[key])
    if (move_cost < 30) {
      move_cost = move_cost + this.speed()
    }
