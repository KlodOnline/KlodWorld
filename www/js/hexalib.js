...
function Cube(array)
    {
    if (array.length!==3) 
        { console.log('Error : incorect format.'); return false; }    
    this.type = 'cube';
    this.x = array[0]*1;
    this.y = array[1]*1;
    this.z = array[2]*1;
    this.stg = function() {return this.x+','+this.y+','+this.z;};
    }
function Axial(array)
    ...
    this.stg = function() {return this.q+','+this.r+','+this.s;};
    }    
function Oddr(array)
    {
    if (array.length!==2) 
    ...
    this.col = array[0]*1;
    this.row = array[1]*1;
    this.stg = function() {return this.col+','+this.row;};
    }
/*------------------------------------------------------------------------------
    Ze Library :
 -----------------------------------------------------------------------------*/
function Hexalib()
    ...
    this.convert = function(coord, cible)
        {
        if (coord.type===cible) {return coord;}
    ...
    this.pixel_to = function(array, target, w, s)
        ...
        // w = WIDTH; s = SIZE
        var x = array[0]*1; var y = array[1]*1;
    ...
    this.common_neighbours = function(coord_a, coord_b)
        ...
        var results = [];
        for (var i=0;i<neighb_a.length;i++) { for(var j=0;j<neighb_b.length;j++)
            {
    ...
    this.cube_direction = function(cube_a, cube_b)
        ...
        for (var i=0; i<this.cube_directions.length; i++)
            { if ( diff.x===this.cube_directions[i].x && diff.y===this.cube_directions[i].y && diff.z===this.cube_directions[i].z )    
                { return i; } }
        return false;    
        };
    // Draws & Fun -------------------------------------------------------------    
    ...
    this.random_line_draw = function (coord_a, coord_b, factor)
        ...
        while ((current.col!==end_place.col || current.row!==end_place.row) && key<normal_line_coords.length*2)
        return results;
        };
    this.ring = function(coord, radius)
        ...
        results.push(this.convert(cube_step, that_sys));
        for (var i=0; i<=5; i++) { for (var j=0; j<radius; j++)
            {
    ...
    this.spiral = function(coord, radius)
        ...
        var results = [coord];
        for (var i=1; i<=radius;i++) { results = results.concat(this.ring(coord, i)); }
        return results;
        };        
    // Arythmetics -------------------------------------------------------------
    ...
    this.cube_scale = function (origin, dir_matrix, d)
        ...
        for (var i=0;i<d;i++){new_coord = this.cube_add(new_coord, dir_matrix);}
        return new_coord;
        };
    ...
    this.cube_distance = function(cube_a, cube_b)
        {
        return (Math.abs(cube_a.x - cube_b.x) + Math.abs(cube_a.y - cube_b.y) + Math.abs(cube_a.z - cube_b.z)) / 2;
        };
