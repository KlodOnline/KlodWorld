...
function PriorityQueue(size = 500) {
  ...
  this.queue_size = function () {
    ...
    for (let key in this.elements) {
      if (this.elements[key].length > 0) {
        size = size + this.elements[key].length
      }
    }
    return size
  }
  ...
  this.put = function (item, priority) {
    ...
    if (typeof this.elements[pkey] === 'undefined') {
      this.elements[pkey] = [element]
    } else {
      this.elements[pkey].push(element)
    }
...
function Mapview(max_col, max_row, start_coord) {
  ...
  this.x_min = 0
  this.x_max = this.max_col * H_W + H_W / 2
  this.y_min = 0
  this.y_max = (this.max_line * H_H * 3) / 4 + H_H / 4
  ...
  LEAFLET_MAP = new L.Map('map', {
    ...
    maxBounds: [
      [this.y_min - H_H, -Infinity],
      [this.y_max + H_H, Infinity]
    ],
  ...
  var imageBounds = [
    [this.y_min * 0.8, this.x_min * 0.8],
    [this.y_max * 1.2, this.x_max * 1.2]
  ]
  ...
  this.refresh = function (e = null) {
    ...
    if (
      this.lastRefresh &&
      performance.now() - this.lastRefresh < 400 &&
      e !== 'moveend' &&
      e !== 'force'
    ) {
      return
  ...
  this.draw_scope = function () {
    ...
    for (const sck of SCOPE) {
      let no_vario = false
      for (const [, obj] of Object.entries(sck)) {
        // Utilisation de `Object.entries` pour éviter de créer un nouvel array
  ...
  this.getHexBounds_NotInCache = function () {
    ...
    for (var col = hb[0]; col <= hb[2]; col++) {
      for (var line = hb[1]; line <= hb[3]; line++) {
        if (
          typeof this.cache[[this.find_normal_col(col, 'getHexBoundnoincache'), line]] ===
          'undefined'
        ) {
          if (col < nic_col_1) {
            nic_col_1 = col
    ...
    if (nic_col_1 !== 9999) {
      return [nic_col_1, nic_line_1, nic_col_2, nic_line_2]
    }
    return false
  }
  /*--------------------------------------------------------------------------
  ...
  /*--------------------------------------------------------------------------
    ...
    --------------------------------------------------------------------------*/
  this.find_road = function (coord) {
    return Bo.road_at(coord)
  }
  this.find_terrain = function (coord) {
    return Rules.getLandInfo(Bo.terrain_at(coord))
  }
  ...
  this.move_cost = function (from_coord, next_coord, is_boat) {
    ...
    if (typeof next_coord === 'undefined') {
      return
    }
    ...
    if (!is_boat && next_terrain.water === true) {
      if (next_road > 0) {
        cost = Rules.getLandInfo(7).move * 1
      }
  ...
  this.move_cost_old = function (from_coord, next_coord, is_boat) {
    ...
    // Doit etre "Parent" de 'ordrexxx.js'->'move_tic'
    //
    if (typeof from_coord === 'undefined' || typeof next_coord === 'undefined') {
      return
    }
    ...
    if (!is_boat && next_terrain.water === true) {
      if (next_road > 0) {
        cost = Rules.getLandInfo(7).move
      } else {
        return Rules.boat_time
      }
    }
  ...
  this.heuristic = function (a, b) {
    ...
    if (col_num > this.max_col / 2) {
      cross = true
    }
    if (cross) {
      if (start.col < goal.col) {
        start.col = start.col + this.max_col
      } else {
        goal.col = goal.col + this.max_col
      }
    }
    let heuristic = Math.abs(start.col - goal.col) + Math.abs(start.row - goal.row)
    return heuristic
  }
  this.neighbours = function (coord) {
    return Bo.neighbours_coords_cache[coord.stg()]
  }
  ...
  this.find_path_OLD = function (start, end) {
    ...
    if (typeof this.cache[cache_txt] !== 'undefined') {
      return this.cache[cache_txt]
    }
    ...
    while (!frontier.is_empty()) {
      ...
      for (var key in neighbours) {
        ...
        var is_better = false
        if (typeof cost_so_far[next.stg()] === 'undefined') {
          is_better = true
        }
        if (new_cost < cost_so_far[next.stg()]) {
          is_better = true
        }
        if (is_better) {
          cost_so_far[next.stg()] = new_cost
...
*/
  ...
  $(document).on('click', function (e) {
    ...
    if (that.shifted) {
      T.add_input(' [' + that.mouse_coord.stg() + '] ')
    } else {
      Se.setStep()
    }
    return false
