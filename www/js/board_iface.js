...
function Board_Iface(max_col, max_row) {
  ...
  this.neighbours_coords = function (coord) {
    ...
    for (const nk in neighb_coords) {
      ...
      if (neighb_coords[nk].row >= this.max_line || neighb_coords[nk].row < 0) {
        delete neighb_coords[nk]
      }
    }
    ...
    while (server_data.length > 0) {
      ...
      while (objects.length > 0) {
        ...
        if (class_name === 'Stack' && first_unit.has(coord_str)) {
          ...
          const new_object = new this.constructors[class_name](
            obj_coord,
            obj,
            this.randval[coord_str]
          )
  ...
  this.road_at = function (coord) {
    const objects = this.objects_at(coord.col, coord.row)
    if (objects['Ground'] != null) {
      if (objects['Ground'].ro * 1 > 0) {
  ...
  this.objects_at = function (col, row) {
    ...
    if (this.objects_by_coords[col + ',' + row] == null) {
      return {}
    }
  ...
  this.uid_add = function (object) {
    this.objects_by_uid[object.uid()] = object
  }
  ...
  this.coord_del = function (object) {
    delete this.objects_by_coords[object.coord.stg()][object.constructor.name]
  }
  this.uid_del = function (object) {
    delete this.objects_by_uid[object.uid()]
  }
  ...
  this.neighbours = function (object) {
    ...
    const neighbours = []
    for (const nc_key in coords_list) {
      neighbours.push(this.objects_at(coords_list[nc_key].col, coords_list[nc_key].row))
    }
    return neighbours
  }
...
function GameObject(coord, json_data, randval) {
  ...
  for (const key_name in json_data) {
    if (key_name !== 't') {
...
*/
  ...
  this.create_layers = function () {
    ...
    const hexa = L.polygon([
      this.pointy_hex_corner(0),
      this.pointy_hex_corner(1),
      this.pointy_hex_corner(2),
      this.pointy_hex_corner(3),
      this.pointy_hex_corner(4),
      this.pointy_hex_corner(5)
    ])
    hexa.setStyle({
      color: null,
      fillColor: '#FFFFFF',
  ...
  this.color_class = function () {
    return helper_color_class(this.co)
  }
  ...
  this.background_colored_square = function (vario = 0) {
    ...
    const corn_4 = [corners[1][0], corners[0][1]]
    const background = L.polygon([corn_1, corn_2, corn_3, corn_4], {
      color: this.co,
      fillOpacity: 1,
      weight: 1
    })
  ...
  this.image_bounds = function () {
    ...
    const x1 = this.center_px[0] - H_W / 2
    const x2 = this.center_px[0] + H_W / 2
    const y1 = this.center_px[1] - H_H / 2
    const y2 = this.center_px[1] + H_H / 2
    return [
      [y1, x1],
      [y2, x2]
    ]
  ...
  this.image_bounds_square = function (vario = 0) {
    ...
    const x1 = this.center_px[0] + vario - (H_W / 6) * 2
    const x2 = this.center_px[0] + vario + (H_W / 6) * 2
    const y1 = this.center_px[1] + vario - (H_W / 6) * 2
    const y2 = this.center_px[1] + vario + (H_W / 6) * 2
    return [
      [y1, x1],
      [y2, x2]
    ]
  ...
  this.pointy_hex_corner = function (side, size = H_S) {
    ...
    data = [
      this.center_px[1] + calc_cache[cache_name][0],
      this.center_px[0] + calc_cache[cache_name][1]
    ]
    return data
  }
...
function Stack(coord, json_data, randval) {
  ...
  this.create_layers = function (no_vario = false) {
    ...
    const layers = []
    for (const uk in this.units) {
      if (this.units[uk].relat_place == undefined) {
        this.units[uk].set_relat_place(this.relat_place)
      }
      // La classe doit dépendre des couleurs du joueur.
      let css_class = 'U ' + this.units[uk].color_class()
      if (Se.isSelected(this.units[uk])) {
        css_class = css_class + ' s'
      }
      if (no_vario) {
        vario = 0
      } else {
      ...
      const picture = L.imageOverlay(
        './u/u' + this.units[uk].ty + '.png',
        this.units[uk].image_bounds_square(parseInt(vario)),
        { className: css_class, idName: 'test', interactive: true, renderer: L.canvas() }
      )
...
function Ground(coord, json_data, randval) {
  ...
  this.create_layers = function () {
    ...
    layers.push(
      L.imageOverlay(url, imageBounds, { className: 'T', interactive: true, renderer: L.canvas() })
    )
    ...
    if (this.id === 13) {
      ...
      if (riverDirections.length > 0) {
        ...
        riverDirections.forEach((direction) => {
          layers.push(
            L.imageOverlay(`./h/r${direction}.png`, imageBounds, {
              className: 'T',
              interactive: true,
              renderer: L.canvas()
            })
      ...
      if (riverDirections.length <= 1) {
        const possibleSources = Bo.find_neighbours_directions(
          this,
          'Ground',
          'id',
        ...
        if (possibleSources.length > 0) {
          ...
          for (const inverseDirection of inverseDirections) {
            if (possibleSources.includes(inverseDirection)) {
              layers.push(
                L.imageOverlay(`./h/s${inverseDirection}.png`, imageBounds, {
                  className: 'T',
                  interactive: true,
                  renderer: L.canvas()
                })
    ...
    if (this.ro > 0) {
      layers.push(
        L.imageOverlay('./h/p6.png', imageBounds, {
          className: 'T',
          interactive: true,
          renderer: L.canvas()
        })
      ...
      roadDirections.forEach((direction) => {
        layers.push(
          L.imageOverlay(`./h/p${direction}.png`, imageBounds, {
            className: 'T',
            interactive: true,
            renderer: L.canvas()
          })
    ...
    if (this.ow != null) {
      ...
      for (let i = 0; i < 6; i++) {
        if (!directions.includes(i)) {
          let c0 = i,
            c1 = (i + 1) % 6 // Use modulo for circular indexing
          segments.push([
            this.pointy_hex_corner(c0, H_S - 0.2),
            this.pointy_hex_corner(c1, H_S - 0.2)
          ])
        }
      }
    ...
    nullDirs.forEach((direction) => {
      layers.push(
        L.imageOverlay(`./h/f${direction}.png`, imageBounds, {
          className: 'F',
          interactive: false,
          renderer: L.canvas()
        })
  ...
  this.create_layers_old = function () {
    ...
    if (this.ro * 1 > 0) {
      ...
      for (const d_key in directions) {
        const road_piece = L.imageOverlay(
          './h/p' + directions[d_key] + '.png',
          this.image_bounds(),
    ...
    if (this.ow != null) {
      ...
      if (segments.length > 0) {
        ...
        layers.push(
          L.polyline(segments, {
            color: Rules.colors[colors_tags[0]],
            opacity: 1,
            weight: 6,
            interactive: false
          })
        ...
        layers.push(
          L.polyline(segments, {
            color: Rules.colors[colors_tags[1]],
            opacity: 1,
            weight: 3,
            interactive: false
          })
    ...
    for (const d_key in null_dirs) {
      const road_piece = L.imageOverlay('./h/f' + null_dirs[d_key] + '.png', this.image_bounds(), {
        className: 'F',
        interactive: false,
        renderer: L.canvas()
      })
  ...
  this.create_layers = function () {
    ...
    if (prev_d.length > 0) {
      const trace = L.polyline(
        [[this.center_px[1], this.center_px[0]], this.side_middle(prev_d[0])],
...
*/
    if (next_d.length > 0) {
      const trace = L.polyline(
        [[this.center_px[1], this.center_px[0]], this.side_middle(next_d[0])],
        { color: '#FF2222', opacity: 0.6, weight: 8, interactive: false }
      )
      layers.push(trace)
    } else {
