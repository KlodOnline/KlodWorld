...
function Menubar() {
    ...
    this.scheduleFetching = function () {
		...
		if (this.recurrentFetching) { clearInterval(this.recurrentFetching); }
    	...
    	this.recurrentFetching = setInterval(() => {
    		...
    		if (this.last_refresh_interval!=this.refresh_interval()) { this.scheduleFetching(); }
    ...
    this.refresh_interval = function() {
    	if (this.sec_to_go()>60) { return 5000; }
    	return 1000;
    }
    // sec to go to the end of the next tic
    this.sec_to_go = function() { return this.next_tic - this.client_time; }
    ...
    this.update_clock = function(next_tic, time) {
		...
		if (this.lock == true) { sec_class = "ld"; } // Orange Class "Locked"
        else { sec_class = "go"; } // Green Class "Ok"
        ...
        $("#server_info")
            .html('<div id="' + sec_class + '">' + display_time + '</div><div id="turn">' + this.last_turn + '</div>');    	
    }
	...
	$('#server_main').on('click', '.quit', function(e) {  
	    Pp.confirmPopup({
	        ...
	        ok: function() { document.location.href = WEBSITE; } // Redirige si confirmé
	    });
	});
