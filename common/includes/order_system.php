...
class OrderFactory
{
    public static function createOrder($order_type, $actorId, $data, $board)
    {
        ...
        if (!$order->validateData()) {
            $order->orderErrorLog("Invalid order data.", true);
            return null;
        }
        if (!$order->actorCanDo()) {
            $order->orderErrorLog("Order impossible for such Object.", true);
            return null;
        }
...
trait GenericOrder
{
    ...
    protected function nextActionTurn(): int
    {
        return 0;
    }
...
class MoveOrder extends OrderRoot
{
    ...
    public function nextActionTurn(): int
    {
        return $this->nextMoveTurn();
    }
    public function totalTurns(): int
    {
        return $this->totalMoveTurns();
    }
    public function validateData(): bool
    {
        return $this->validateMoveData();
    }
    public function actorCanDo(): bool
    {
        return $this->actorCanMove();
    }
    public function action()
    {
        $this->move();
...
class RoadOrder extends OrderRoot
{
    ...
    public function nextActionTurn(): int
    {
        return $this->nextBuildRoadTurn();
    }
    public function validateData(): bool
    {
        return $this->validateBuildRoadData();
    }
    public function actorCanDo(): bool
    {
        return $this->actorCanBuildRoad();
    }
    public function action()
    {
...
trait CanBuildCity
{
    ...
    {
        $this->orderErrorLog('Checking order list...');
        if (!$this->actor()->haveOrder('BUILD_CITY')) {
            return false;
        }
        return true;
    }
