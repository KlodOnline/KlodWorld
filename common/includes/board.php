...
class Board
{
    ...
    ***************************************************************************/
    ...
    public function updateCollection($object)
    {
        ...
        if ($object->isLocatable()) {
            $this->addToLocationsIndex($object);
        }
    }
    ...
    public function deleteFromCollection($object)
    {
        ...
        $class = get_class($object);
        if ($object->isLocatable()) {
            $this->deleteFromLocationsIndex($object);
        }
        $collection = &$this->getCollectionByClass($class);
    ...
    private function deleteFromLocationsIndex($object)
    {
        ...
        if (isset($this->locationsIndex[$key])) {
            ...
            if (empty($this->locationsIndex[$key])) {
                unset($this->locationsIndex[$key]);
            }
        }
    }
    /***************************************************************************
    ...
    ***************************************************************************/
    ...
    public function getObjectByKey($class, $key = [])
    {
        ...
        if (isset($collection[$searchKey])) {
            return $collection[$searchKey];
        }
    ...
    */
    private function loadByColumnsAndValues($objectClass, $columns = ['id'], $values = [[null]])
    {
        // $columns Validation
        if (!is_array($columns)) {
            logMessage('Error: $columns must be an array.');
        }
        // $values Validation
    ...
    public function saveCollection($objectClass)
    {
        ...
        foreach ($collection as $key => $object) {
            if ($object !== null) {
                ...
                if ($cacheObject === null) {
                    logMessage('Warning ! No cache for '.$objectClass.' key='.$key);
                }
    ...
    public function allAreClose($locatables)
    {
        ...
        for ($i = 0; $i < count($locatables) - 1; $i++) {
            ...
            for ($j = $i + 1; $j < count($locatables); $j++) {
                ...
                if (!$this->hexalib->isNeighbour($A_coord, $B_coord)) {
                    return false;
                }
            }
        }
    ...
    {
        if (!isset($this->Player) or $this->Player === null) {
            return 0;
        }
    ...
    public function findOwner($object)
    {
        ...
        if (isset($object->meta_id)) {
            return $object;
        }
        if (isset($object->player_id)) {
            return $this->getObjectByKey('Player', [$object->player_id]);
        }
        if (isset($object->owner_id)) {
            return $this->getObjectByKey('Player', [$object->owner_id]);
        }
        return null;
    }
